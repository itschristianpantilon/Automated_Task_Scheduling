...
const TaskRow = ({ taskType, title, duration, onPress, threeDotBtn, status, role }) => {
  ...
  return (
    <TouchableOpacity onPress={onPress}>
      <View className={`px-4 space-y-6 rounded-md mb-2 border border-gray-400 hover:border-secondary-100 min-h-[15vh] ${status === 'Finished' ? 'border-green-500 hidden' : ''} ${status === 'Inactive' ? 'border-red-500' : ''}`}>
